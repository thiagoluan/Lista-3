...
// prototipo da função subtracao
int **subtracao(int **a, int **b, int n); 
...
int main(void){
    ...
    // chama a função que realiza a subtracao entre as matrizes
    int **matriz=subtracao(a, b, n); 
    printf("A subtração dos elementos de A - B \n");
    for(i=0;i<n;i++){
    	printf("\n");
        for(j=0;j<n;j++){
            printf(" %d  ", matriz[i][j]);
...
// função que realiza a subtracao entre as matrizes A e B
int **subtracao(int **a, int **b, int n){ 
    int i, j;
    int **resul;
    ...
    //laço de repetição que passa pelos valores das matrizes e realiza a subtracao
    for( i=0;i<n;i++){
        for(j=0;j<n;j++){ 
